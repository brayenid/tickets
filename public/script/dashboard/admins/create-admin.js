...
const createAdminForm = $('#create-admin')

const addAdminBtn = document.querySelector('#add-admin-btn')

const addAdminLoading = (isLoading) => {
  if (isLoading) {
    ...
    >
      ...
      `
    addAdminBtn.setAttribute('disabled', '')
  } else {
    addAdminBtn.innerHTML = 'Tambah'
...
const addAdmin = async () => {
  ...
  const adminPhone = $('#phone').value
  const adminAddress = $('#address').value
  const adminBirth = $('#birth').value
  ...
  const { isConfirmed } = await Swal.fire({
    ...
    text: 'Kamu yakin menambah admin baru?',
